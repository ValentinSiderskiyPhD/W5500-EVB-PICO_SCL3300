...
#include <Wire.h>
 ...
 // -----------------------------------------------------------------
// Return codes for begin():
#define AD569X_OK               0   // success
#define AD569X_ERR_I2C_INIT     1   // i2c_dev->begin() failed
#define AD569X_ERR_MODE         2   // setMode() failed
// -----------------------------------------------------------------
...
public:
  Adafruit_AD569x();
  uint8_t begin(uint8_t addr, TwoWire *wire = &Wire);
  ...
  bool setMode(ad569x_operating_modes newmode, bool enable_ref, bool gain2x);
    ...
    /**
   * After a failed write or setMode(), retrieve the raw I2C error code:
